package cn.lcy.xxmall.filter.backstage;
...
public class LoginIntecepter implements HandlerInterceptor{
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o) throws Exception {
        ...
        String token = httpServletRequest.getParameter("token");        // 获取前端发送过来的token认证
        String adminId = httpServletRequest.getParameter("adminId");    // 获取前端发送过来的管理员id
        Jedis jedis = JedisUtil.getJedis();
        List<String> adminInfo = jedis.hmget("admin_"+adminId,"token","endTime");   // get(0)返回token， get(1)返回时间  Long.valueOf(adminInfo.get(1)

        if( adminInfo.size()>0 && adminInfo.get(0)!=null && adminInfo.get(0).equals(token) ){
            Integer dateState = DateUtil.compareTimeStamp( DateUtil.getDate() , DateUtil.getDate( Long.valueOf(adminInfo.get(1) ))); // 判断token有效时间是否过期;
            // 如果token有效时间没有过期
